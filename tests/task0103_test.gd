...
## TASK-010-3 Night tactical camera transition 검증.
## DAY/NIGHT phase 전환에 따라 Player 직접 이동 입력 활성화/비활성화가 올바르게
## 적용되고, 카메라가 DAY(day_zoom) ↔ NIGHT(night_zoom) 사이를 전환하는지 자동 검증한다.
...
func _process(_delta: float) -> bool:
	...
	match _phase:
		...
		TestPhase.INIT_DAY:
			...
			_check(_player._night_mode == false, "player starts in DAY (direct control enabled)")
			_check(_zoom_near(_player.day_zoom), "camera zoom starts at day_zoom (%.2f)" % _camera.zoom.x)
		...
		TestPhase.DAY_MOVE:
			...
			if _elapsed() >= MOVE_WAIT_FRAMES:
				...
				_check(_player.global_position.x > _start_x, "DAY: player can move right (direct control)")
		...
		TestPhase.TO_NIGHT:
			...
			if _elapsed() >= 4:
				...
				_check(_player._night_mode == true, "NIGHT: direct movement input disabled (flag set)")
		...
		TestPhase.DAY_MOVE_AGAIN:
			...
			if _elapsed() >= MOVE_WAIT_FRAMES:
				_release_move("move_right")
				_check(_player.global_position.x > _start_x, "DAY again: player can move (control restored)")
		...
		TestPhase.CYCLE_AGAIN:
			if _elapsed() >= ZOOM_WAIT_FRAMES:
				...
				_game_time.advance(10.0)
				_check(_player._night_mode == true, "repeated cycle: NIGHT mode toggles on again")
				_enter(TestPhase.CYCLE_ZOOM_END)
		TestPhase.CYCLE_ZOOM_END:
			if _elapsed() >= ZOOM_WAIT_FRAMES:
				_check(_zoom_near(_player.night_zoom), "repeated cycle: camera converges to night_zoom (no accumulation) (%.2f)" % _camera.zoom.x)
				_game_time.advance(10.0)
				_check(_player._night_mode == false, "repeated cycle: DAY mode toggles back")
